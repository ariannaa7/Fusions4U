...
# Output: output_spanning_reads.txt w/ standard SAM fields, plus fusion_ID
...
# Author: Völundur Hafstað
# GitHub: https://github.com/VolundurH/wgs_fusion_pipeline

# Updated on: 2024-12-11
# Author: Arianna Alamshahi
# Update notes:
  # Removed singularity dependency
  # Removed requirement of pre-existing directory called "output"
...
disc_reads %>% 
  write_tsv("discordant_read_pairs.txt")
...
dir.create("softclip_fasta_files")
...
softclips %>% 
  ...
  pwalk(~write_tsv(..2, file = paste0("softclip_fasta_files/disc_read_cluster_", ..1, ".fa"), col_names = F))
  
# Write the files to output
disc_read_cluster_key %>% 
  write_tsv("discordant_read_cluster_linkage_key.txt")

softclips %>% 
  write_tsv("softclipped_reads.txt")

disc_read_clusters %>% 
  write_tsv("discordant_read_clusters.txt")

disc_read_cluster_bed %>% 
  mutate(start = start-1) |> 
  write_tsv("discordant_read_clusters.bed", col_names = F)


# Use bedtools to create fasta sequences ----

system2(command = "bedtools", args = paste0("getfasta -fi ",ref_genome, " -bed discordant_read_clusters.bed -nameOnly > discordant_read_cluster_sequences.fa"))

# Split discordant_read_cluster_sequences.fa into multiple smaller fasta files

disc_read_sequences <-  read_lines("discordant_read_cluster_sequences.fa") %>% 
  as_tibble()

dir.create("discordant_read_cluster_sequences")

disc_read_sequences %>% 
  mutate(grp = str_extract(value, "(?<=\\>).+")) %>% 
  fill(grp) %>%
  nest(.by = grp) %>% 
  pwalk(~write_tsv(..2, file = paste0("discordant_read_cluster_sequences/", ..1, ".fa"), col_names = F ))

# Index these multiple smaller fasta files with novoindex

print("Indexing...")

list.files("discordant_read_cluster_sequences/") %>% 
  as_tibble() %>% 
  filter(!str_detect(value, ".ndx$")) %>% 
  mutate(
    cmd = "novoindex", 
    args = paste0("discordant_read_cluster_sequences/", value, ".ndx discordant_read_cluster_sequences/", value)) %>% 
...
# one in discordant_read_cluster_sequences and one in softclip_fasta_files.
...
# Example: We align the reads in softclip_fasta_files/disc_read_cluster_2_1.fa locally to discordant_read_cluster_sequences/2_1.fa 
print("Performing alignments...")

file.create("softclip_alignment.txt")
file.create("softclip_alignment_log.txt")
...
list.files("discordant_read_cluster_sequences/") %>% 
  ...
  mutate(cmd = "novoalign",
    args = paste0("-f softclip_fasta_files/disc_read_cluster_", value, " -d discordant_read_cluster_sequences/", value, ".ndx -r ALL -o SAM -h-1-1 -l6 >> softclip_alignment.txt 2>> softclip_alignment_log.txt" )) %>% 
  pwalk(~system2(command = ..2, args = ..3))

alignment <- read_tsv("softclip_alignment.txt", comment = "@", col_names = sam_fields) %>% 
  filter(MAPQ != 0) %>% 
...
summary_table %>% 
  write_tsv("fusion_support_summary_table.txt")
...
softclip_summary_table %>% 
  write_tsv("fusion_summary_table_breakpoint_supporting_reads.txt")
